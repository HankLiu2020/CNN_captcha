...
def userTest(model, dataLoader):
    totalNum = 0
    errNum = 0
    for cnt, input in enumerate(dataLoader, 0):
        '''if cnt >= 200:
            break'''
        totalNum += 1
        x, label = input
        ...
        realLabel = LabeltoStr(np.array(label[0],dtype=np.int8))
        # print(label,realLabel)
        y1, y2, y3, y4 = model(x.cuda())
        ...
        is_true=realLabel == decLabel
        if not(is_true):
            print("real: %s -> %s , %s" % (realLabel, decLabel, str(is_true)))
            errNum+=1
        
    print("\n total %s, right %s" % (totalNum, totalNum-errNum))
